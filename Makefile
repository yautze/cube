...
build:
	docker build --no-cache -t $(IMAGE_NAME):$(DATE_TAG)-$(IMAGE_VERSION) --progress=plain --build-arg IMAGE_VERSION=${IMAGE_VERSION} -f packages/cubejs-docker/release.Dockerfile . 2>&1 | tee build.log

store-build:
	docker build --no-cache -t $(STORE_IMAGE_NAME):$(DATE_TAG)-$(IMAGE_VERSION) --progress=plain --build-arg IMAGE_VERSION=${IMAGE_VERSION} -f rust/cubestore/Dockerfile ./rust 2>&1 | tee store_build.log
