...
RUN DEBIAN_FRONTEND=noninteractive \
    ...
    && apt-get install -y --no-install-recommends libssl3 curl \
       cmake pip python3 python3.11 libpython3.11-dev gcc g++ make cmake openjdk-17-jdk-headless \
...
# install extension 
ADD packages/cubejs-docker/duckdb_extension/httpfs.duckdb_extension_v1_3_2 /root/.duckdb/extensions/v1.3.2/linux_amd64/httpfs.duckdb_extension
ENV DEV_BUILD_TAG=202508251358-dev
COPY packages/cubejs-backend-native /cube/node_modules/@cubejs-backend/native

COPY packages/cubejs-docker/jar/hsqldb-2.7.1.jar /cube/node_modules/@cubejs-backend/jdbc/drivers-10.17/hsqldb.jar
